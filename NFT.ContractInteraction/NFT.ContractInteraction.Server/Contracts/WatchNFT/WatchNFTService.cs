...
namespace Watches.Contracts.WatchNFT
{
    public partial class WatchNFTService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, WatchNFTDeployment watchNFTDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<WatchNFTDeployment>().SendRequestAndWaitForReceiptAsync(watchNFTDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, WatchNFTDeployment watchNFTDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<WatchNFTDeployment>().SendRequestAsync(watchNFTDeployment);
        }

        public static async Task<WatchNFTService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, WatchNFTDeployment watchNFTDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, watchNFTDeployment, cancellationTokenSource);
            return new WatchNFTService(web3, receipt.ContractAddress);
        }
        ...
        public WatchNFTService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
        ...
        public Task<string> SafeMintRequestAsync(string to, BigInteger tokenId, BigInteger royaltyAmount, string royaltyRecipient)
        {
            var safeMintFunction = new SafeMintFunction();
                safeMintFunction.To = to;
                safeMintFunction.TokenId = tokenId;
                safeMintFunction.RoyaltyAmount = royaltyAmount;
                safeMintFunction.RoyaltyRecipient = royaltyRecipient;
            
             return ContractHandler.SendRequestAsync(safeMintFunction);
        }

        public Task<TransactionReceipt> SafeMintRequestAndWaitForReceiptAsync(string to, BigInteger tokenId, BigInteger royaltyAmount, string royaltyRecipient, CancellationTokenSource cancellationToken = null)
        {
            var safeMintFunction = new SafeMintFunction();
                safeMintFunction.To = to;
                safeMintFunction.TokenId = tokenId;
        ...
        public Task<string> SafeMintAndSetUriRequestAsync(SafeMintAndSetUriFunction safeMintAndSetUriFunction)
        {
             return ContractHandler.SendRequestAsync(safeMintAndSetUriFunction);
        }

        public Task<TransactionReceipt> SafeMintAndSetUriRequestAndWaitForReceiptAsync(SafeMintAndSetUriFunction safeMintAndSetUriFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(safeMintAndSetUriFunction, cancellationToken);
        }

        public Task<string> SafeMintAndSetUriRequestAsync(string to, BigInteger tokenId, string uri, BigInteger royaltyAmount, string royaltyRecipient)
        {
            var safeMintAndSetUriFunction = new SafeMintAndSetUriFunction();
                safeMintAndSetUriFunction.To = to;
                safeMintAndSetUriFunction.TokenId = tokenId;
                safeMintAndSetUriFunction.Uri = uri;
                safeMintAndSetUriFunction.RoyaltyAmount = royaltyAmount;
                safeMintAndSetUriFunction.RoyaltyRecipient = royaltyRecipient;
            
             return ContractHandler.SendRequestAsync(safeMintAndSetUriFunction);
        }

        public Task<TransactionReceipt> SafeMintAndSetUriRequestAndWaitForReceiptAsync(string to, BigInteger tokenId, string uri, BigInteger royaltyAmount, string royaltyRecipient, CancellationTokenSource cancellationToken = null)
        {
            var safeMintAndSetUriFunction = new SafeMintAndSetUriFunction();
                safeMintAndSetUriFunction.To = to;
                safeMintAndSetUriFunction.TokenId = tokenId;
